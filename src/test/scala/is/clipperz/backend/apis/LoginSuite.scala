...
object LoginSpec extends ZIOSpec[UserArchive & BlobArchive]:
  override def bootstrap: ZLayer[Scope, Any, UserArchive & BlobArchive] = 
    UserArchive.fs(userBasePath, 2) ++ BlobArchive.fs(blobBasePath, 2)
  ...
  val saveUser: ZIO[UserArchive & BlobArchive, Throwable, Unit] =
    ZIO
      ...
      .flatMap((userArchive, blobArchive) =>
        userArchive.saveUser(testUser, false).flatMap(_ =>
          blobArchive.saveBlob(HexString(indexCardReference), 
                                ZStream.fromIterable(HexString(indexCardContent).toByteArray)).map(_ => ())
        )
      )
  ...
  def loginRequestStep1(c: String, stepData: String, withSession: Boolean): Request =
    Request(
      url = URL(!! / "login" / "step1" / c ),
      method = Method.POST,
      headers = if (withSession) Headers((SessionManager.sessionKeyHeaderName, sessionKey)) else Headers.empty,
      data = HttpData.fromString(stepData, StandardCharsets.UTF_8.nn),
      version = Version.Http_1_1,
    )

  def loginRequestStep2(c: String, stepData: String, withSession: Boolean): Request =
    Request(
  ...
  def spec = suite("BlobApis")(
    ...
    test("Login step 1, 2 - fail - no sessionKey") {
      ...
      for {
        ...
        m1 <- srpFunctions.computeM1(c.toByteArray, 
