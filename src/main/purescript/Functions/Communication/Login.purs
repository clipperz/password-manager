...
login       (OfflineProxy _)                  _    _       _ _ = throwError $ UnhandledCondition "TODO"
login proxy@(OnlineProxy _ _ maybeSessionKey) hash srpConf c p = do

  sessionKey <- liftAff $ flip fromMaybe maybeSessionKey <$> (fromArrayBuffer <$> SRP.randomArrayBuffer 32)
  
  let connectionState = {
    hashFunc: hash
  , proxy: updateSessionKey proxy $ Just sessionKey
  }

  Tuple newProxy loginStep1Result@{ s } <- loginStep1 connectionState srpConf c
  Tuple newProxy' { m1, kk, m2, userInfoReferences } <- loginStep2 (connectionState {proxy = newProxy}) srpConf c p loginStep1Result
...
loginStep1 :: ConnectionState -> SRPConf -> HexString -> ExceptT AppError Aff (Tuple Proxy LoginStep1Result)
loginStep1 connectionState srpConf c = do
  ...
  Tuple newProxy step1Response <- manageGenericRequest connectionState url POST (Just body) RF.json
  ...
  if bb == fromInt (0)
    ...
    else pure $ Tuple newProxy { aa, a, s: responseBody.s, bb }
...
loginStep2 :: ConnectionState -> SRPConf -> HexString -> HexString -> LogintStep2Data -> ExceptT AppError Aff (Tuple Proxy LoginStep2Result)
loginStep2 connectionState srpConf c p { aa, bb, a, s } = do
  ...
  Tuple newProxy step2Response <- manageGenericRequest connectionState url POST (Just body) RF.json
  ...
  pure $ Tuple newProxy { m1, kk, m2: responseBody.m2, userInfoReferences }
